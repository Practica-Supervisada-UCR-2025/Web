...
export async function POST(request: NextRequest) {
    try {
        ...
        const backendUrl = `${process.env.NEXT_PUBLIC_POST_URL}/api/admin/reported/restore`;
        ...
        if (contentType && contentType.includes('application/json')) {
            ...
            backendData = { success: true, message: 'Post and its reports have been successfully restored' };
        }
        ...
        if (backendResponse.status === 200) {
            return NextResponse.json(
                {
                    success: true,
                    message: 'Post and its reports have been successfully restored'
                },
                { status: 200 }
            );
        ...
        console.error('Error restoring reported post:', error);
        return NextResponse.json(
            {
                success: false,
                message: 'mensaje de error del servicio'
